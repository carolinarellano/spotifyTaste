...
def exportData(playlists, index, sp):
    ...
    features_df.to_json(filename, orient='columns', indent=4)

    print('playlist' + str(index) + ' json file exported') 


credentials = json.load(open('spotifyTaste\credentials.json'))
...
playlists = json.load(open('spotifyTaste\playlistsYochi.json'))
